...
def getFileSelectionName(emotiontype, emotionscatter, curcount):
    if (emotiontype == 'N' or emotiontype == 'NS' or emotiontype == 'NOTSMILE'):
        nsemotions = ['AMAZED', 'LOOKING', 'DISGUISTING', 'YAWNING', 'CALM']
...
def getSessionDir(emotiontype):
    ...
    elif (emotiontype == 'D' or emotiontype == 'DISGUISTING'):
        sessionlist = ['/session03'] # Disguisting photos are contained only in session 1 and 3
...
def getRecordingDir(pathtosubject, emotiontype):
    ...
    elif (emotiontype == 'L' or emotiontype == 'LOOKING' or emotiontype == 'D' or emotiontype == 'DISGUISTING' or
                  emotiontype == 'Y' or emotiontype == 'YAWNING'):
        return '/03' # Looking, disguisting and yawning photos are shooted only in recording 3
    else:
    # if emotiontype != 'C' or emotiontype != 'CALM'
        ...
        if (emotiontype == 'C' or emotiontype == 'CALM'):
            ...
            if str(pathtosubject).find('session03') != -1:
                recordingdirs.remove('03')  # except folders with disguisting photos
...
def getFullEmotionName(emotiontype):
    ...
    if (emotiontype == 'L' or emotiontype == 'LOOKING'):
        return 'LOOKING'
    if (emotiontype == 'D' or emotiontype == 'DISGUISTING'):
        return 'DISGUISTING'
...
def createSelectionFolder(foldername):
    pathtodir = os.getcwd() + '\\' + foldername # form full path to th folder with selection
...
emotionlist = ['S', 'SMILE', 'N', 'NS', 'NOTSMILE', 'A', 'AMAZED', 'L', 'LOOKING', 'D', 'DISGUISTING' ,'Y', 'YAWNING',
               'C', 'CALM']
