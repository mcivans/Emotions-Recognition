...
addpath('D:/_Repositories/GenderclassificationCAL/Learning db');
listing = dir('D:/_Repositories/GenderclassificationCAL/Learning db');
% Путь к svmLib
%addpath D:/libsvm-3.21/matlab
addpath('D:/libsvm-3.20/matlab');
% Путь к VL_FEAT
% run('VLFEAT/toolbox/vl_setup')
run('D:\vlfeat-0.9.20-bin\vlfeat-0.9.20\toolbox\vl_setup')
% Размеры изображения
he = 60;
wi = 60;
...
for i = 3:length(listing);
    ...
    if isempty(findstr(listing(i).name,'_02_'))
        lbl(cntr) = 1;
    else
        lbl(cntr) = 0;
    end;
...
GABORT = readtable('D:/_Repositories/GenderclassificationCAL/FACES_PREDICT_14_04/GABORS.csv','ReadVariableNames',false);
...
%GABORT = readtable('D:/_Repositories/GenderclassificationCAL/FACES_PREDICT_14_04/GABORS.csv');
